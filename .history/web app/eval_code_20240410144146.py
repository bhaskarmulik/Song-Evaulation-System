...
class Audio_eval:
    ...
    def pitch_features(self,frames, sr):
        ...
        for frame in frames:
            ...
            pmed.append(slef.l2_norm(medsubp))
        l2n = pd.Series(l2n)
        pitch_der = l2n.diff()
    ...
    def final_merge(uploaded_file, original_file):
        ...
        user_audio = separator.predict(openunmix.utils.preprocess(torch.as_tensor(user_song).float()),
                                            targets=['vocals'],
                                            residual=True,
                                            rate = sr1,
                                            device=device,
                                            model_str_or_path='umxhq')
        og_audio = separator.predict(openunmix.utils.preprocess(torch.as_tensor(original_song).float()),
                                            targets=['vocals'],
                                            residual=True,
                                            rate = sr2,
                                            device=device,
