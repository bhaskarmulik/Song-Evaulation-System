...
class Audio_eval:
    ...
    def final_merge(self,uploaded_file, original_file):
        ...
        user_audio = openunmix.predict(openunmix.utils.preprocess(torch.as_tensor(user_song).float()),
                                            targets=['vocals'],
                                            residual=True,
                                            rate = sr1,
                                            device=device,
                                            model_str_or_path='umxhq')
        og_audio = openunmix.predict(openunmix.utils.preprocess(torch.as_tensor(original_song).float()),
                                            targets=['vocals'],
                                            residual=True,
                                            rate = sr2,
                                            device=device,
