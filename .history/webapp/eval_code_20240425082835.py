...
class Audio_eval:
    ...
    def find_peak_intensity(intensity_db, lower_freq, higher_freq, sample_rate):
    ...
    def singing_power_ratio(self,audio_data, sample_rate):
        ...
        low_intensity_db = self.find_peak_intensity(intensity_db, lower_freq, sample_rate)
        high_intensity_db = self.find_peak_intensity(intensity_db, higher_freq, sample_rate)
    ...
    def final_merge(self,uploaded_file, original_file):
        ...
        self.final_err =  w1*self.mfcc_errors + w2*self.energy_errors + w3*self.rop_err +  w4*self.ratio + w5*self.pitch_err
