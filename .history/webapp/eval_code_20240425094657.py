...
class Audio_eval:
    ...
    def pitch_err(self, user_pitch_f, og_pitch_f):
        vec1 = vec1[~np.isnan(user_pitch_f)]
        vec2 = vec2[~np.isnan(og_pitch_f)]
    ...
    def final_merge(self,uploaded_file, original_file):
        ...
        self.final_err =  w1*self.mfcc_errors + w2*self.energy_errors + w3*self.rop_err +  w4*self.ratio + w5*self.pitch_error
